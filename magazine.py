...
def country_kb():
    ...
    for code, c in COUNTRY_PRICES.items():
        if c.get("only_autoreg"):
            text = f"{c['name']} — {c['price_autoreg']}₽"
            if c.get('warning'):
                text += " ⚠️"
            kb.inline_keyboard.append([InlineKeyboardButton(text=text, callback_data=f"cnt_{code}")])
        else:
            text = f"{c['name']} — А:{c['price_autoreg']}₽ / Ф:{c['price_phishing']}₽"
            if c.get('warning'):
                text += " ⚠️"
            kb.inline_keyboard.append([InlineKeyboardButton(text=text, callback_data=f"cnt_{code}")])
...
async def select_country(c: types.CallbackQuery, state: FSMContext):
    ...
    if country.get("only_autoreg"):
        ...
        product_name = f"{country['name']} (Авторег)"
    ...
    # Для остальных стран — показываем выбор типа с разными ценами
    original_price_autoreg = country["price_autoreg"]
    original_price_phishing = country["price_phishing"]
    
    # Рассчитываем цены со скидкой
    if discount > 0:
        price_autoreg = int(original_price_autoreg * (100 - discount) / 100)
        price_phishing = int(original_price_phishing * (100 - discount) / 100)
        discount_text = f"\n\n🎫 *Применена скидка {discount}%*"
    else:
    ...
    await state.update_data(
        country_code=code,
        country_name=country["name"],
        price_autoreg=price_autoreg,
        price_phishing=price_phishing,
    ...
    text = (
        ...
        f"🤖 Авторег — {price_autoreg}₽"
    )
    if price_phishing is not None:
        text += f"\n🎣 Фишинг — {price_phishing}₽"
    text += discount_text
...
async def select_account_type(c: types.CallbackQuery, state: FSMContext):
    ...
    if type_ == "autoreg":
        price = data.get("price_autoreg")
        original_price = data.get("original_price_autoreg")
        type_label = "Авторег"
    else:
        price = data.get("price_phishing")
        original_price = data.get("original_price_phishing")
        type_label = "Фишинг"
    
    full_product_name = f"{country_name} ({type_label})"
    await state.update_data(
    ...
    discount_text = f"\n\n🎫 *Применена скидка {discount}%*\n~~{original_price}₽~~ → *{price}₽*" if discount > 0 else f"\n💰 *Итого к оплате:* {price}₽"
    warning = f"\n\n{COUNTRY_PRICES.get(data.get('country_code'), {}).get('warning', '')}"
    
    await c.message.edit_text(
        f"✅ *Страна:* {country_name} — {original_price}₽{warning}{discount_text}\n\n"
        f"📬 *Введите ваш Telegram username для доставки аккаунта:*\nПример: @durov",
        parse_mode="Markdown"
    )
    await c.answer()
...
async def select_recipient(c: types.CallbackQuery, state: FSMContext):
    ...
    if recipient_type == "self":
        # Если есть username - сразу переходим к выбору количества
        if c.from_user.username:
            ...
            # Если нет username - просим ввести
        ...
        # Покупка другу - всегда просим username
        await state.set_state(OrderForm.stars_username)
        await c.message.edit_text(
...
async def get_stars_username(m: types.Message, state: FSMContext):
    ...
    if recipient_type == "self":
        ...
        # Отправляем уведомление другу (если есть такой пользователь)
        try:
            friend_username = stars_username.replace("@", "")
            # Пробуем отправить уведомление другу
            await bot.send_message(
                m.from_user.id,
                f"📨 *Уведомление*\n\n"
                f"Вы купили звёзды для @{friend_username}!\n"
...
async def main():
    ...
    logger.info("Бот запущен (обновлённые цены, выбор типа для аккаунтов)")
