...
# Кэш статистики (обновляется раз в 60 секунд)
_stats_cache = {}
_stats_cache_time = 0

def get_db():
    """Возвращает соединение с БД (для каждого потока своё)"""
...
def init_db():
    with get_db() as conn:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_stars_status ON stars_orders(status)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_stars_user ON stars_orders(user_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_promocodes_code ON promocodes(code)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_user_promocodes_user ON user_promocodes(user_id)")
init_db()
...
async def profile(c: types.CallbackQuery):
    account_discount = get_user_discount(c.from_user.id, "account")
    stars_discount = get_user_discount(c.from_user.id, "stars")
    text = (
        f"👤 *Личный кабинет*\n\n"
...
# -------------------- Просмотр подтверждённых / отклонённых --------------------
...
async def main():
    ...
    logger.info("Бот запущен (оптимизированная версия с индексами и кэшем)")
