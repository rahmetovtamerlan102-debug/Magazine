...
def currency_kb(product_type="account"):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💎 Доллары (USDT)", callback_data=f"currency_usd_{product_type}")],
        [InlineKeyboardButton(text="◀ Назад", callback_data="back_to_country")]
    ])

# Кнопка оплаты через @send (без API ключей)
def send_payment_kb(order_id, order_type, amount_usd):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text="💳 Оплатить через @send",
            url="https://t.me/send"
        )],
        [InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"paid_{order_type}_{order_id}")],
...
async def select_currency(c: types.CallbackQuery, state: FSMContext):
    ...
    if currency == "usd":
        ...
        await c.message.edit_text(
            ...
            f"💎 *Оплата в долларах (USDT)*\n\n"
            f"💰 Сумма: *{final_price} ${discount_text}*\n\n"
            f"📌 *Инструкция:*\n"
            f"1️⃣ Нажмите кнопку «💳 Оплатить через @send»\n"
            f"2️⃣ Оплатите USDT через @send\n"
            f"3️⃣ Вернитесь и нажмите «✅ Я оплатил»\n\n"
            f"📬 Доставка будет на ваш username",
            parse_mode="Markdown",
            reply_markup=send_payment_kb(order_id, "account", final_price)
        )
        return
...
async def select_stars(c: types.CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        ...
        f"💳 *Выберите способ оплаты:*",
        parse_mode="Markdown",
        reply_markup=currency_kb("stars")
    )
    await c.answer()

@dp.callback_query(OrderForm.currency_choice, F.data.startswith("currency_"))
async def select_currency_stars(c: types.CallbackQuery, state: FSMContext):
    data = c.data.split("_")
    currency = data[1]
    ...
    if currency == "usd":
        ...
        await c.message.edit_text(
            ...
            f"3️⃣ Вернитесь и нажмите «✅ Я оплатил»",
            parse_mode="Markdown",
            reply_markup=send_payment_kb(order_id, "stars", final_price)
        )
        return
...
async def main():
    ...
    logger.info("Бот запущен с кнопкой отзывов и оплатой через @send")
