...
# АДМИНИСТРАТОРЫ (ВАШ ID И ID КОЛЛЕГИ)
...
def main_menu():
    ...
    # Но здесь мы не знаем user_id, поэтому добавим в обработчике /start отдельно
...
async def cmd_start(message: types.Message):
    ...
    # Если админ — добавляем кнопку админ-панели в меню
...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    # Отправляем уведомления ВСЕМ администраторам в ЛС
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            # Пробуем отправить хотя бы текст без фото
            try:
...
async def admin_all_orders(callback: types.CallbackQuery):
    ...
    for order in orders[:20]:
        status_emoji = '🔄' if order[8] == 'waiting_confirmation' else '✅'
        text += f"{status_emoji} #{order[0]} | {order[5]}₽ | @{order[2] or 'no name'}\n"
    text += f"\n📊 *Всего:* {len(orders)}"
