...
# НОВАЯ ФУНКЦИЯ: страны в две колонки
def country_kb():
    kb = InlineKeyboardMarkup(inline_keyboard=[])
    # Получаем список всех ключей стран
...
async def get_stars_username(m: types.Message, state: FSMContext):
    ...
    if recipient_type == "self":
        ...
        # Уведомление при покупке звёзд другу
        try:
            await m.answer(
                f"📨 *Уведомление*\n\n"
...
async def main():
    ...
    logger.info("Бот запущен с обновлёнными ценами и клавиатурой в 2 колонки")
