...
# ==================== КЛАВИАТУРЫ ====================
def main_menu(is_admin_user: bool = False):
    """Обычное меню магазина. Для админа добавляется кнопка перехода в админ-панель."""
    buttons = [
...
async def cmd_start(message: types.Message):
    ...
    # Админам не нужна подписка, обычным пользователям – проверяем
    ...
    # Для всех показываем обычное меню (админы увидят кнопку перехода в админку)
    await message.answer(
...
async def back_main(callback: types.CallbackQuery):
    """Возвращает в обычное меню магазина (для админов – с кнопкой перехода в админку)."""
    await callback.message.edit_text(
        "🌟 *Магазин аккаунтов*\n\nВыберите действие:",
        parse_mode="Markdown",
        reply_markup=main_menu(is_admin(callback.from_user.id))
...
async def open_admin_panel(callback: types.CallbackQuery):
    """Переход в админ-панель (только для администраторов)."""
...
async def buy_number(callback: types.CallbackQuery, state: FSMContext):
    # Проверяем подписку для обычных пользователей (админы могут покупать без подписки)
...
async def admin_pending_orders(callback: types.CallbackQuery):
    ...
    # Удаляем исходное сообщение, чтобы не было дублирования
...
async def process_block_user(message: types.Message, state: FSMContext):
    ...
    if current_state == "waiting_for_block_user":
        try:
            ...
            await state.clear()
            await cmd_start(message)  # возвращаем в обычное меню
        except ValueError:
...
async def confirm_order_admin(callback: types.CallbackQuery):
    ...
    # Обновляем сообщение с фото (если возможно)
