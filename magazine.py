...
def init_db_sync():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS stars_orders (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER,
        username TEXT,
        stars INTEGER,
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS user_promocodes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER,
        promo_code TEXT,
        discount_percent INTEGER,
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_stars_status ON stars_orders(status)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_stars_user ON stars_orders(user_id)")
    conn.commit()
    conn.close()

init_db_sync()  # Выполняем при старте

# ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ БД ====================
async def log_admin_action(admin_id, action, target=""):
    q = "INSERT INTO admin_logs (admin_id, action, target) VALUES (?,?,?)"
    await run_query(q, (admin_id, action, target), fetch=False)

async def get_admin_logs(limit=30):
    q = "SELECT admin_id, action, target, timestamp FROM admin_logs ORDER BY id DESC LIMIT ?"
    return await run_query(q, (limit,))
...
async def get_all_orders_by_status(status, limit=None, offset=None):
    if limit is not None and offset is not None:
        ...
        return await run_query(q, (status, limit, offset))
    else:
        q = "SELECT * FROM orders WHERE status = ? ORDER BY id DESC"
        return await run_query(q, (status,))

async def get_orders_count_by_status(status):
    res = await run_query("SELECT COUNT(*) FROM orders WHERE status = ?", (status,))
    return res[0][0] if res else 0

async def get_order(oid):
    res = await run_query("SELECT * FROM orders WHERE id = ?", (oid,))
    return res[0] if res else None

async def get_all_orders_for_export():
    return await run_query("SELECT * FROM orders ORDER BY id DESC")
...
async def save_stars_order(uid, uname, stars, price, stars_username):
    ...
    return await run_query(q, (uid, uname, stars, price, stars_username), fetch=False)
...
async def get_stars_orders_count_by_status(status):
    res = await run_query("SELECT COUNT(*) FROM stars_orders WHERE status = ?", (status,))
    return res[0][0] if res else 0

async def get_stars_order(oid):
    res = await run_query("SELECT * FROM stars_orders WHERE id = ?", (oid,))
    return res[0] if res else None

async def get_user_stars_orders(uid):
    return await run_query("SELECT * FROM stars_orders WHERE user_id = ? ORDER BY id DESC", (uid,))

async def get_all_stars_orders_for_export():
    return await run_query("SELECT * FROM stars_orders ORDER BY id DESC")
...
async def create_promocode(code, discount_percent, product_type, max_uses):
    ...
    try:
        await run_query(q, (code.upper(), discount_percent, product_type, max_uses), fetch=False)
        return True
    except:
        return False
...
async def get_all_promocodes_cached():
    ...
    if time.time() - _promos_cache_time > 60:
        _promos_cache = await run_query("SELECT * FROM promocodes ORDER BY id DESC")
        _promos_cache_time = time.time()
    return _promos_cache

async def delete_promocode(code):
    await run_query("DELETE FROM promocodes WHERE code = ?", (code.upper(),), fetch=False)
    global _promos_cache_time
    _promos_cache_time = 0

async def activate_promocode_for_user(user_id, code, product_type):
    promo = await run_query("SELECT code, discount_percent, max_uses, used_count FROM promocodes WHERE code = ? AND product_type = ?", (code.upper(), product_type))
    if not promo:
    ...
    user_has = await run_query("SELECT 1 FROM user_promocodes WHERE user_id = ? AND promo_code = ? AND product_type = ?", (user_id, code.upper(), product_type))
    if user_has:
        return False, "Вы уже активировали этот промокод"
    await run_query("INSERT INTO user_promocodes (user_id, promo_code, discount_percent, product_type) VALUES (?,?,?,?)", (user_id, code.upper(), promo[1], product_type), fetch=False)
    await run_query("UPDATE promocodes SET used_count = used_count + 1 WHERE code = ?", (code.upper(),), fetch=False)
    global _promos_cache_time
    _promos_cache_time = 0
...
async def get_user_discount(user_id, product_type):
    res = await run_query("SELECT MAX(discount_percent) FROM user_promocodes WHERE user_id = ? AND product_type = ?", (user_id, product_type))
...
async def get_stats():
    ...
    if time.time() - _stats_cache_time < 60 and _stats_cache:
        return _stats_cache
    total_accounts = (await run_query("SELECT COUNT(*) FROM orders"))[0][0]
    total_accounts_sum = (await run_query("SELECT SUM(price) FROM orders WHERE status='completed'"))[0][0] or 0
    pending_accounts = (await run_query("SELECT COUNT(*) FROM orders WHERE status='pending'"))[0][0]
    processing_accounts = (await run_query("SELECT COUNT(*) FROM orders WHERE status='processing'"))[0][0]
    completed_accounts = (await run_query("SELECT COUNT(*) FROM orders WHERE status='completed'"))[0][0]
    rejected_accounts = (await run_query("SELECT COUNT(*) FROM orders WHERE status='rejected'"))[0][0]
    total_stars = (await run_query("SELECT COUNT(*) FROM stars_orders"))[0][0]
    total_stars_sum = (await run_query("SELECT SUM(price) FROM stars_orders WHERE status='completed'"))[0][0] or 0
    pending_stars = (await run_query("SELECT COUNT(*) FROM stars_orders WHERE status='pending'"))[0][0]
    processing_stars = (await run_query("SELECT COUNT(*) FROM stars_orders WHERE status='processing'"))[0][0]
    completed_stars = (await run_query("SELECT COUNT(*) FROM stars_orders WHERE status='completed'"))[0][0]
    rejected_stars = (await run_query("SELECT COUNT(*) FROM stars_orders WHERE status='rejected'"))[0][0]
    blocked_count = (await run_query("SELECT COUNT(*) FROM blocked"))[0][0]
    _stats_cache = {
...
async def block_user(uid):
    await run_query("INSERT OR IGNORE INTO blocked (user_id) VALUES (?)", (uid,), fetch=False)

async def unblock_user(uid):
    await run_query("DELETE FROM blocked WHERE user_id = ?", (uid,), fetch=False)

async def is_blocked(uid):
    res = await run_query("SELECT 1 FROM blocked WHERE user_id = ?", (uid,))
    return len(res) > 0
...
async def get_all_users_cached():
    ...
    if time.time() - _users_cache_time < 300:
        return _users_cache
    users = set()
    for row in await run_query("SELECT DISTINCT user_id FROM orders"):
        users.add(row[0])
    for row in await run_query("SELECT DISTINCT user_id FROM stars_orders"):
        users.add(row[0])
...
# ==================== ПРОСМОТР ЗАКАЗОВ (АККАУНТЫ И ЗВЁЗДЫ) ====================
...
# ==================== FLASK ДЛЯ HEALTHCHECK И SELF-PING ====================
...
async def main():
    ...
    logger.info("Бот запущен (polling + асинхронная БД)")
