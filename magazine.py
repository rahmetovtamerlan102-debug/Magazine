...
# ========== ГЛАВНЫЙ ОБРАБОТЧИК /start (с поддержкой заказов с сайта) ==========
...
async def cmd_start(
    ...
    if payload and payload.startswith("order_"):
        try:
            ...
            if len(parts) >= 3:
                country = parts[0]      # 🇺🇸 США (USA)
                price = parts[1]        # 80
                username = parts[2]     # @angel7
            else:
                country = "товар"
                price = "?"
                username = order_data

            # Показываем пользователю его заказ
            await message.answer(
                f"🛒 *ЗАКАЗ ИЗ МАГАЗИНА*\n\n"
                f"📱 Товар: {country}\n"
                f"💰 Сумма: {price} ₽\n"
                f"👤 Ваш Telegram: {username}\n\n"
                f"📸 *Теперь отправьте СКРИНШОТ чека* для подтверждения оплаты.\n\n"
            ...
            await message.answer(
                f"📌 *Как отправить чек?*\n"
                f"1️⃣ Сделайте скриншот экрана оплаты\n"
                f"2️⃣ Нажмите «📎» → «Фото»\n"
                f"3️⃣ Выберите скриншот и отправьте\n\n"
                f"⚡ *Бот примет заказ в обработку сразу после получения чека!*",
                parse_mode="Markdown"
            )
            
            # Сохраняем временные данные заказа в FSM
            from aiogram.fsm.context import FSMContext
            state = dp.fsm.get_context(bot, message.from_user.id, message.chat.id)
            price_int = int(price) if price.isdigit() else 0
            await state.update_data(temp_order={
            ...
            logger.error(f"Ошибка обработки order payload: {e}")
            await message.answer(
                "❌ Ошибка обработки заказа.\n"
                "Пожалуйста, оформите заказ заново на сайте."
            )
            return
...
# ========== ОБРАБОТЧИКИ ДЛЯ ПОКУПОК В БОТЕ ==========
...
# 1. Для заказов, сделанных через бота (с состоянием OrderForm.waiting_for_screenshot)
@dp.message(StateFilter(OrderForm.waiting_for_screenshot), F.photo)
async def process_screenshot_from_bot(message: types.Message, state: FSMContext):
    logger.info("Скриншот от бота (заказ через кнопки)")
...
# 2. Для заказов с сайта (без состояния, но с temp_order)
@dp.message(F.photo)
async def process_screenshot_from_website(message: types.Message, state: FSMContext):
    logger.info(f"Скриншот от {message.from_user.id} (возможно с сайта)")
    ...
    # Если есть temp_order — значит заказ с сайта
    if data and 'temp_order' in data:
        temp_order = data['temp_order']
        
        order_id = await save_order(
            message.from_user.id,
            message.from_user.username or "без username",
            temp_order.get('product_type', 'number'),
            temp_order.get('product_name', 'Виртуальный номер'),
            temp_order.get('product_price', 0),
            temp_order.get('product_details', '')
        )
        ...
        await message.answer(
            f"🔄 Ваш заказ #{order_id} принят и обрабатывается!\n\n"
            f"📦 {temp_order['product_name']}\n"
            f"💰 Сумма: {temp_order['product_price']} ₽\n\n"
            f"Ожидайте подтверждения оплаты.\n"
            f"📞 {SUPPORT_USERNAME}"
        )
        ...
        await notify_admins_new_order(
            order_id=order_id,
            user_display=user_display,
            price=temp_order['product_price'],
            product_name=temp_order['product_name']
        )
    else:
        # Нет активного заказа — предлагаем оформить
        await message.answer(
            "❌ Я не могу обработать этот скриншот.\n\n"
            "Пожалуйста, сначала оформите заказ:\n"
            "1️⃣ Через сайт — выберите товар и нажмите «Оплатил → бот»\n"
            "2️⃣ Или через бота — нажмите кнопку «Купить номер» или «Купить звёзды»\n\n"
            "После этого отправьте скриншот чека."
        )
...
# ========== АДМИН ФУНКЦИИ (статистика, поиск, блокировка, рассылка и т.д.) ==========
