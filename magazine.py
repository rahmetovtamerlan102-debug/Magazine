...
ADMIN_IDS = [8276815852, 8840342301]
...
# ========== OCR С ЗАЩИТОЙ ОТ ОШИБОК ==========
OCR_AVAILABLE = True
try:
    from PIL import Image
    import pytesseract
    import io
    logger.info("✅ OCR модули загружены")
except ImportError as e:
    OCR_AVAILABLE = False
    logger.warning(f"⚠️ OCR недоступен: {e}")

async def verify_receipt(file_id: str, expected_amount: int) -> tuple[bool, str]:
    """Проверяет чек через OCR, если недоступен — отправляет на ручную проверку"""
    
    if not OCR_AVAILABLE:
        return False, "OCR недоступен, заказ отправлен на ручную проверку"
    ...
    try:
        ...
        # 1. Фейковые слова
        fake_words = ["ТЕСТ", "ПРИМЕР", "ДЕМО", "SAMPLE", "TEST", "ФЕЙК"]
        for word in fake_words:
            if word in text_upper:
                return False, f"Чек содержит запрещённое слово '{word}'"
        
        # 2. Ключевые слова оплаты
        payment_words = ["ПЕРЕВОД", "ОПЛАТА", "СПИСАНИЕ", "КВИТАНЦИЯ"]
        if not any(word in text_upper for word in payment_words):
            return False, "Чек не содержит слов подтверждения оплаты"
        
        # 3. Проверка номера телефона
        phone_patterns = [
            r'\+7\s*902\s*736\s*57\s*59',
            r'8\s*902\s*736\s*57\s*59',
            r'902\s*736\s*57\s*59',
        ]
        phone_found = any(re.search(p, text) for p in phone_patterns)
        if not phone_found:
            return False, "В чеке не найден номер получателя (+79027365759)"
        
        # 4. Проверка ФИО
        name_patterns = [r'ЕГОР', r'ЕГОР\s+О\.', r'ЕГОР\s+ОЛЕГОВИЧ']
        name_found = any(re.search(p, text_upper) for p in name_patterns)
        if not name_found:
            return False, "В чеке не найдено имя получателя (Егор)"
        ...
        if found_amount is None:
            return False, "Не удалось распознать сумму в чеке"
        
        if found_amount != expected_amount:
            return False, f"Сумма {found_amount}₽ не совпадает с {expected_amount}₽"
        
        return True, "Чек успешно проверен"
        
    except Exception as e:
        logger.error(f"Ошибка OCR: {e}")
        return False, f"Техническая ошибка: заказ отправлен на ручную проверку"
...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    # === ПРОВЕРКА ЧЕКА ЧЕРЕЗ OCR ===
    is_valid, reason = await verify_receipt(photo.file_id, data['product_price'])
    
    if is_valid:
        confirm_order(order_id)
        await message.answer(
            f"✅ *ЗАКАЗ #{order_id} ОПЛАЧЕН И ПОДТВЕРЖДЁН!*\n\n"
            f"📦 {data['product_name']}\n"
        ...
        await message.answer(
            ...
            f"Причина: {reason}\n\n"
            f"Отправьте настоящий чек оплаты.\n"
            f"Если вы оплатили, обратитесь в поддержку: {SUPPORT_USERNAME}",
            parse_mode="Markdown"
        )
...
async def main():
    ...
    if OCR_AVAILABLE:
        logger.info("✅ OCR включён, чеки проверяются автоматически")
    else:
        logger.warning("⚠️ OCR недоступен, все заказы идут на ручную проверку")
