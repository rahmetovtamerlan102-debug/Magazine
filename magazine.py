...
async def upload_check(request):
    """Принимает скриншот с сайта и отправляет админам через бота"""
    try:
        ...
        order_id = await save_order(
            user_id=0,  # Временно, потом можно доработать определение user_id
            username=user_id,
            product_type="number",
            product_name=product_name,
        ...
        # Сохраняем ID скриншота, если нужно
        # await update_order_screenshot(order_id, "from_website")
