...
async def init_db_pool():
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute('ALTER TABLE orders ADD COLUMN IF NOT EXISTS original_price INTEGER DEFAULT 0')
        await conn.execute('ALTER TABLE orders ADD COLUMN IF NOT EXISTS discount_percent INTEGER DEFAULT 0')
        ...
        ''')
        # Добавляем колонки если их нет
        await conn.execute('ALTER TABLE stars_orders ADD COLUMN IF NOT EXISTS original_price INTEGER DEFAULT 0')
        await conn.execute('ALTER TABLE stars_orders ADD COLUMN IF NOT EXISTS discount_percent INTEGER DEFAULT 0')
...
async def process_user_promo(m: types.Message, state: FSMContext):
    ...
    if success:
        await m.answer(f"✅ {message}\n\n⚠️ Чтобы применить скидку, начните оформление заказа заново через /start", parse_mode="Markdown")
...
async def select_stars(c: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(order_id=order_id, order_type="stars", stars=stars_data["stars"], price=price)
    await state.set_state(OrderForm.screenshot)
...
async def main():
    ...
    logger.info("Бот запущен (инлайн-клавиатура, PostgreSQL, скидки)")
