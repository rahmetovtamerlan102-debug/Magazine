...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    # Отправляем короткое уведомление админам
    for admin_id in ADMIN_IDS:
        try:
            ...
            order_type = "аккаунта" if data['product_type'] == "number" else "звёзд"
            text = f"📦 Новый заказ {order_type} #{order_id} от {username} на {data['product_price']}₽"
...
async def admin_pending_orders(callback: types.CallbackQuery):
    ...
    if not orders:
        ...
        return
    for order in orders:
        oid, uid, uname, ptype, pname, price, details, screenshot, status, created = order[:10]
        text = f"📋 *ЗАЯВКА #{oid}*\n\n👤 *Пользователь:* @{uname or 'нет username'} [ID: {uid}]\n📦 *Товар:* {pname}\n💰 *Сумма:* {price} ₽\n🕐 *Создан:* {created}\n\n📸 *Чек:*"
        await callback.message.answer_photo(screenshot, caption=text, parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="✅ ПРИНЯТЬ", callback_data=f"confirm_{oid}")],
...
async def confirm_order_admin(callback: types.CallbackQuery):
    ...
    confirm_order(oid)
    with sqlite3.connect(DB_NAME) as conn:
        row = conn.execute("SELECT user_id, product_price, product_name FROM orders WHERE id = ?", (oid,)).fetchone()
    if row:
        uid, price, pname = row
        await bot.send_message(uid, f"✅ *ЗАКАЗ #{oid} ПРИНЯТ!*\n\n📦 {pname}\n💰 Сумма: {price} ₽\n🎉 Спасибо за покупку!\n📞 {SUPPORT_USERNAME}", parse_mode="Markdown")
    await callback.answer(f"✅ Заказ #{oid} принят")
