...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    short_text = f"📦 Новый заказ {order_type} #{order_id} от @{message.from_user.username or 'без_username'} на {data['product_price']}₽"

    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(admin_id, short_text)
