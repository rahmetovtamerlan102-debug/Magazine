...
# Premium планы (сначала выбор, потом добавляем цену страны)
PREMIUM_PLANS = {
...
async def orders(c: types.CallbackQuery):
    ...
    text = "📦 *Мои заказы*\n\n"
    for o in ords:
        status_emoji = "✅" if o[10] == "completed" else "⏳"
        text += f"{status_emoji} #{o[0]} | {o[7]}₽ | {o[4]}\n"
...
async def buy(c: types.CallbackQuery, state: FSMContext):
    if not await is_subscribed(c.from_user.id):
        ...
        return
    await state.set_state(Order.premium)
    await c.message.edit_text("💰 *ШАГ 1: ВЫБОР PREMIUM*\n\nВыберите вариант:", parse_mode="Markdown", reply_markup=premium_kb())
...
async def select_country(c: types.CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(
        ...
        f"📬 *ШАГ 3: КОНТАКТ ДЛЯ ДОСТАВКИ*\n"
        f"Введите ваш Telegram username (например, @durov):",
        parse_mode="Markdown"
    )
    await c.answer()
...
# ==================== ЗАПУСК ====================
async def main():
    # Запускаем Flask в отдельном потоке для Healthcheck + самопинг
