...
class OrderForm(StatesGroup):
    ...
    stars_recipient_type = State()  # 1. Сначала: себе или другу
    stars_amount = State()           # 2. Потом: количество звёзд
    stars_username = State()         # 3. Если другу: запрос username
    screenshot = State()
...
async def select_recipient(c: types.CallbackQuery, state: FSMContext):
    recipient_type = c.data.split("_")[1]  # "self" или "friend"
    await state.update_data(recipient_type=recipient_type)
    
    if recipient_type == "self":
        # Купить себе — проверяем username
...
async def get_stars_username(m: types.Message, state: FSMContext):
    ...
    if recipient_type == "self":
        # Это случай, когда у пользователя не было username
        await state.update_data(stars_username=stars_username)
        await state.set_state(OrderForm.stars_amount)
        await m.answer(
            f"✅ *Начисление на:* {stars_username} (ваш аккаунт)\n\n"
        ...
        # Купить другу — сохраняем username друга и переходим к выбору количества
        await state.update_data(stars_username=stars_username)
        await state.set_state(OrderForm.stars_amount)
        await m.answer(
...
async def main():
    ...
    logger.info("Бот запущен. Порядок покупки звёзд: сначала получатель, потом количество")
