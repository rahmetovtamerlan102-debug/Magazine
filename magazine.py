...
class OrderForm(StatesGroup):
    ...
    stars_amount = State()
    stars_recipient_type = State()  # себе или другу
...
def stars_recipient_kb():
    """Клавиатура выбора: Купить себе / Купить другу"""
    return InlineKeyboardMarkup(inline_keyboard=[
...
async def select_recipient(c: types.CallbackQuery, state: FSMContext):
    ...
    if recipient_type == "self":
        ...
        if not stars_username:
            # Если нет username, запрашиваем
...
async def get_stars_username(m: types.Message, state: FSMContext):
    ...
    recipient_text = "ваш аккаунт" if data.get("recipient_type") == "self" else f"аккаунт {stars_username}"
    await m.answer(
        f"💳 *ЗАКАЗ ЗВЁЗД #{order_id}*\n"
        f"⭐ {data['stars']} Stars\n"
        f"💰 Сумма: {data['price']}₽\n"
