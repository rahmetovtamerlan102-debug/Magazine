...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS user_promocodes (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER,
            promo_code TEXT,
            discount_percent INTEGER,
...
# ==================== ПРОМОКОДЫ ====================
def create_promocode(code, discount_percent, product_type, max_uses):
    with sqlite3.connect(DB_NAME) as conn:
        try:
            conn.execute("INSERT INTO promocodes (code, discount_percent, product_type, max_uses) VALUES (?,?,?,?)",
...
def activate_promocode_for_user(user_id, code, product_type):
    with sqlite3.connect(DB_NAME) as conn:
        promo = conn.execute("SELECT code, discount_percent, max_uses, used_count FROM promocodes WHERE code = ? AND product_type = ?", (code.upper(), product_type)).fetchone()
        if not promo:
            return False, "Промокод не найден"
        if promo[2] != 0 and promo[3] >= promo[2]:
            return False, "Промокод уже использован максимальное количество раз"
        user_has = conn.execute("SELECT 1 FROM user_promocodes WHERE user_id = ? AND promo_code = ? AND product_type = ?", (user_id, code.upper(), product_type)).fetchone()
        if user_has:
            return False, "Вы уже активировали этот промокод"
        conn.execute("INSERT INTO user_promocodes (user_id, promo_code, discount_percent, product_type) VALUES (?,?,?,?)",
                    (user_id, code.upper(), promo[1], product_type))
        conn.execute("UPDATE promocodes SET used_count = used_count + 1 WHERE code = ?", (code.upper(),))
        return True, f"Промокод {code.upper()} активирован! Скидка {promo[1]}% на {'аккаунты' if product_type == 'account' else 'звёзды'}"

def get_user_discount(user_id, product_type):
    with sqlite3.connect(DB_NAME) as conn:
        result = conn.execute("SELECT MAX(discount_percent) FROM user_promocodes WHERE user_id = ? AND product_type = ?", (user_id, product_type)).fetchone()[0]
        return result or 0
...
# ==================== КЛАВИАТУРЫ ====================
# ГЛАВНОЕ МЕНЮ - REPLY КЛАВИАТУРА (всегда внизу)
def main_kb(user_id):
    buttons = [
...
# ==================== ОСНОВНЫЕ ХЕНДЛЕРЫ (ReplyKeyboard) ====================
...
async def buy_account(m: types.Message, state: FSMContext):
    if not await is_subscribed(m.from_user.id):
        await m.answer("Подпишитесь на канал!", reply_markup=main_kb(m.from_user.id))
        return
    await state.set_state(OrderForm.account_country)
    await m.answer("🌍 *Выберите страну аккаунта:*", parse_mode="Markdown", reply_markup=country_kb())

@dp.message(F.text == "⭐ Купить звёзды")
async def buy_stars(m: types.Message, state: FSMContext):
    if not await is_subscribed(m.from_user.id):
        await m.answer("Подпишитесь на канал!", reply_markup=main_kb(m.from_user.id))
        return
...
async def profile(m: types.Message):
    account_discount = get_user_discount(m.from_user.id, "account")
    stars_discount = get_user_discount(m.from_user.id, "stars")
    text = (
        f"👤 *Личный кабинет*\n\n"
    ...
    await m.answer(text, parse_mode="Markdown", reply_markup=profile_kb())

@dp.message(F.text == "📦 Мои заказы")
async def my_orders(m: types.Message):
    account_orders = get_orders(m.from_user.id)
    stars_orders = get_user_stars_orders(m.from_user.id)
    if not account_orders and not stars_orders:
        await m.answer("У вас пока нет заказов", reply_markup=main_kb(m.from_user.id))
        return
    text = "📦 *Мои заказы*\n\n"
    if account_orders:
        text += "📱 *Аккаунты:*\n"
...
async def support(m: types.Message):
    await m.answer(f"🆘 Поддержка: {SUPPORT_USERNAME}", reply_markup=main_kb(m.from_user.id))
...
async def admin_panel(m: types.Message):
    if m.from_user.id not in ADMIN_IDS:
        ...
        return
    await m.answer("🔐 *Админ панель*", parse_mode="Markdown", reply_markup=admin_main_kb())

# ==================== ХЕНДЛЕРЫ ЛИЧНОГО КАБИНЕТА И ПРОМОКОДОВ ====================
...
async def process_user_promo(m: types.Message, state: FSMContext):
    ...
    success, message = activate_promocode_for_user(m.from_user.id, code, "account")
    if not success:
        success, message = activate_promocode_for_user(m.from_user.id, code, "stars")
    if success:
        await m.answer(f"✅ {message}", parse_mode="Markdown", reply_markup=main_kb(m.from_user.id))
        log_admin_action(m.from_user.id, "activate_promo", f"{code}")
    else:
        await m.answer(f"❌ {message}", parse_mode="Markdown", reply_markup=main_kb(m.from_user.id))
    await state.clear()

# ==================== ВЫБОР СТРАНЫ (АККАУНТ) ====================
...
async def select_country(c: types.CallbackQuery, state: FSMContext):
    ...
    discount = get_user_discount(user_id, "account")
    if discount > 0:
...
async def get_account_address(m: types.Message, state: FSMContext):
    ...
    if not addr.startswith("@"):
        addr = "@" + addr
    if len(addr) < 3:
        await m.answer("❌ Слишком короткий username. Пример: @durov", reply_markup=ReplyKeyboardRemove())
        return
    data = await state.get_data()
    if not data.get("product_name"):
        await m.answer("❌ Ошибка. Начните заново /start", reply_markup=main_kb(m.from_user.id))
        await state.clear()
        return
    final_price = data.get("price", data["original_price"])
    order_id = save_order(m.from_user.id, m.from_user.username or "no_username", data["product_name"], final_price, addr)
...
# ==================== ПОКУПКА ЗВЁЗД (ВЫБОР ПОЛУЧАТЕЛЯ, КОЛИЧЕСТВА) ====================
...
async def select_stars(c: types.CallbackQuery, state: FSMContext):
    ...
    user_id = c.from_user.id
    discount = get_user_discount(user_id, "stars")
    if discount > 0:
        price = int(original_price * (100 - discount) / 100)
        discount_text = f"\n\n🎫 *Применена скидка {discount}%*"
        price_text = f"\n💰 *Было:* {original_price}₽\n💰 *Стало:* {price}₽"
    ...
    order_id = save_stars_order(c.from_user.id, c.from_user.username or "no_username", stars_data["stars"], price, stars_username)
...
async def wrong_screenshot(m: types.Message):
    await m.answer("❌ Отправьте именно ФОТО (скриншот чека).", reply_markup=ReplyKeyboardRemove())

# ==================== АДМИН-ПАНЕЛЬ (ВСЕ ХЕНДЛЕРЫ) ====================
...
async def promo_get_max_uses(m: types.Message, state: FSMContext):
    ...
    if create_promocode(code, discount, product_type, max_uses):
        ...
        log_admin_action(m.from_user.id, "create_promo", f"{code} ({discount}%, {product_text})")
    else:
...
async def process_delete_promo(m: types.Message, state: FSMContext):
    ...
    if exists:
        ...
        log_admin_action(m.from_user.id, "delete_promo", code)
    else:
...
async def main():
    ...
    logger.info("Бот запущен с ReplyKeyboardMarkup (главное меню внизу)")
