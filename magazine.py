...
async def save_order(user_id, username, product_type, product_name, price, details, source="bot"):
    db = await get_db()
...
async def notify_admins_new_order(order_id, user_display, price, product_name):
...
async def my_orders(callback: types.CallbackQuery):
    ...
    for o in orders:
        ...
        text += f"#{o['id']} | {o['product_price']}₽ | {created}\n{status}\n\n"
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀ НАЗАД", callback_data="back_main")]]))
    await callback.answer()
    ...
    if query.isdigit():
        ...
        if orders:
            ...
            for o in orders[:10]:
                ...
                created = datetime.fromtimestamp(o['created_at']).strftime("%d.%m %H:%M")
                text += f"{emoji} #{o['id']} | {o['product_price']}₽ | {created} @{o['username'] or 'no name'}\n"
...
async def admin_export(callback: types.CallbackQuery):
    ...
    writer.writerow(["ID", "User ID", "Username", "Товар", "Цена", "Статус", "Создан", "Чек ID"])
    for o in orders:
        writer.writerow([o['id'], o['user_id'], o['username'], o['product_name'], o['product_price'], o['status'], datetime.fromtimestamp(o['created_at']).strftime("%Y-%m-%d %H:%M:%S"), o['screenshot_file_id'] or "-"])
...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    await notify_admins_new_order(order_id, user_display, data['product_price'], data['product_name'])
...
async def main():
    ...
    # КРИТИЧНО: удаляем вебхук перед запуском polling
    try:
