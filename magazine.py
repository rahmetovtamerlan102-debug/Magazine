...
async def cmd_start(message: types.Message):
    ...
    logger.info(f"START MESSAGE: {message.text}")

    parts = message.text.split(maxsplit=1)
    # Обработка заказа с сайта (deep-link)
    if len(parts) == 2 and parts[1].startswith("order_"):
        # Извлекаем текст заказа, убирая "order_"
        order_text = unquote(parts[1][6:])   # 6 — длина "order_"
        # Отправляем уведомление всем администраторам
        for admin_id in ADMIN_IDS:
            await bot.send_message(admin_id, f"🛒 НОВЫЙ ЗАКАЗ С САЙТА!\n\n{order_text}")
        await message.answer("✅ Заказ с сайта принят! Отправьте скриншот чека.")
        return

    # Если параметров нет — обычное начало работы бота
...
# Остальные обработчики (проверка подписки, back_main, admin_panel, поддержка,
# buy_number, select_country, buy_stars, select_stars, process_screenshot,
# invalid_screenshot, my_orders, admin_all_orders, admin_block,
# process_block_user, confirm_order_admin, reject_order_admin)
# оставлены без изменений (они уже есть в предыдущих версиях).
# Для краткости они не переписаны заново, но предполагаются присутствующими.
