...
# Состояние для активации промокода пользователем
...
async def process_user_promo(m: types.Message, state: FSMContext):
    ...
    # Пытаемся активировать сначала для аккаунтов, потом для звёзд (подойдёт любой)
...
async def main():
    ...
    logger.info("Бот запущен с поддержкой промокодов для аккаунтов и звёзд")
