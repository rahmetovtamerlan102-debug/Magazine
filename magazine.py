...
async def cmd_start(
    ...
    if not payload:
        await message.answer(
            ...
            "Чтобы сделать заказ, перейдите на сайт и нажмите «Оплатил → бот».\n\n"
            "📌 Вы также можете купить номера или звёзды прямо здесь, нажав на кнопки ниже.",
    ...
    if payload.startswith("order_"):
        try:
            ...
            # Разбираем строку: страна|цена|username
...
async def process_block_user(message: types.Message, state: FSMContext):
    try:
        ...
        await state.clear()
        await cmd_start(message, command=CommandObject(args=None))  # Передаём пустой command
    except ValueError:
