...
# ========== ХРАНИЛИЩЕ СЕССИЙ (одноразовые ключи) ==========
pending_sessions = {}  # {session_id: {"user_id": ..., "expires": ..., "used": False}}
...
async def cors_middleware(request, handler):
    """Разрешает запросы с любого домена (для сайта)"""
...
async def register_session(request):
    """Сайт вызывает этот эндпоинт перед отправкой в бота"""
    try:
        ...
        logger.info(f"Создана сессия {session_id[:8]}... для {user_id}")
...
async def run_aiohttp():
    ...
    logger.info(f"✅ Эндпоинт /register_session доступен")
...
async def cmd_start(message: types.Message, command: CommandObject):
            ...
            if len(parts) >= 4:
                ...
                if not is_valid_session:
                    await message.answer(
                        ...
                        "Этот заказ не может быть обработан.\n"
                        "Пожалуйста, оформите заказ **только через наш сайт**:\n"
                        "🔗 https://howscad-market.onrender.com\n\n"
                        "❗ Не копируйте и не вставляйте ссылки вручную.",
                        parse_mode="Markdown"
                    )
                    return
                
                # ✅ ЗАКАЗ С САЙТА ПРИНЯТ
                ...
                # ПЕРЕВОДИМ В СОСТОЯНИЕ ОЖИДАНИЯ СКРИНА
                ...
                await message.answer(
                    f"📌 *Как отправить чек?*\n"
                    f"1️⃣ Переведите {price} ₽ на номер +79027365759 (Егор)\n"
                    f"2️⃣ Сделайте скриншот экрана оплаты\n"
                    f"3️⃣ Нажмите «📎» → «Фото» и отправьте скриншот\n\n"
                    f"⚡ *После получения чека заказ уйдёт на проверку админу!*",
                    parse_mode="Markdown"
                )
                return
            else:
                await message.answer(
                    "❌ *НЕПРАВИЛЬНЫЙ ФОРМАТ ЗАКАЗА*\n\n"
                    "Пожалуйста, оформите заказ **только через наш сайт**:\n"
                    "🔗 https://howscad-market.onrender.com",
                    parse_mode="Markdown"
                )
    ...
    # ===== ЛЮБОЙ ДРУГОЙ PAYLOAD =====
    await message.answer(
        "❌ *НЕИЗВЕСТНАЯ КОМАНДА*\n\n"
        "Заказы принимаются **ТОЛЬКО через наш сайт**:\n"
        "🔗 https://howscad-market.onrender.com",
        parse_mode="Markdown"
    )
...
async def process_screenshot(message: types.Message, state: FSMContext):
    logger.info(f"Скриншот от {message.from_user.id} в состоянии waiting_for_screenshot")
...
async def main():
    global _timeout_started, _cleaner_started
    await init_db()
