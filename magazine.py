...
async def save_order(uid, uname, product_name, price, addr):
    async with db_pool.acquire() as conn:
        return await conn.fetchval(
            "INSERT INTO orders (user_id, username, product_type, product_name, price, address) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id",
...
async def save_stars_order(uid, uname, stars, price, stars_username):
    async with db_pool.acquire() as conn:
        return await conn.fetchval(
            "INSERT INTO stars_orders (user_id, username, stars, price, stars_username) VALUES ($1, $2, $3, $4, $5) RETURNING id",
...
async def activate_promocode_for_user(user_id, code, product_type):
    async with db_pool.acquire() as conn:
        ...
        user_has = await conn.fetchval("SELECT 1 FROM user_promocodes WHERE user_id = $1 AND promo_code = $2 AND product_type = $3", user_id, code.upper(), product_type)
        if user_has:
            return False, "Вы уже активировали этот промокод"
        await conn.execute("INSERT INTO user_promocodes (user_id, promo_code, discount_percent, product_type) VALUES ($1, $2, $3, $4)",
...
async def get_user_discount(user_id, product_type):
    async with db_pool.acquire() as conn:
        res = await conn.fetchval("SELECT MAX(discount_percent) FROM user_promocodes WHERE user_id = $1 AND product_type = $2", user_id, product_type)
        return res or 0
...
async def my_orders(c: types.CallbackQuery):
    ...
    if account_orders:
        ...
        for o in account_orders:
            ...
            text += f"{status_emoji} *#{o['id']}* | {o['price']}₽ | {o['product_name']} | {status_text} | {created}\n"
    if stars_orders:
        text += "\n⭐ *Звёзды:*\n"
        for o in stars_orders:
            ...
            status_text = "Оплачено" if o['status'] == "completed" else "На проверке" if o['status'] == "processing" else "Отклонён"
            created = o['created'].strftime("%d.%m %H:%M")
            text += f"{status_emoji} *#{o['id']}* | {o['price']}₽ | {o['stars']}⭐ | {status_text} | {created}\n"
...
async def export_accounts(c: types.CallbackQuery):
    ...
    for o in orders:
        writer.writerow([o['id'], o['user_id'], o['username'], o['product_name'], o['price'], o['address'], o['status'], o['created'].strftime("%Y-%m-%d %H:%M:%S")])
    output.seek(0)
...
async def export_stars(c: types.CallbackQuery):
    ...
    for o in orders:
        writer.writerow([o['id'], o['user_id'], o['username'], o['stars'], o['price'], o['stars_username'], o['status'], o['created'].strftime("%Y-%m-%d %H:%M:%S")])
    output.seek(0)
    file = io.BytesIO(output.getvalue().encode('utf-8-sig'))
...
async def admin_accounts_processing(c: types.CallbackQuery):
    ...
    for o in orders:
        text += f"┌ #{o['id']} | {o['price']}₽ | @{o['username'] or o['user_id']} (id:{o['user_id']})\n└ 📬 {o['address']}\n\n"
    kb = InlineKeyboardMarkup(inline_keyboard=[])
    for o in orders:
...
async def view_account_order(c: types.CallbackQuery):
    ...
    if not order:
        ...
        return
    caption = f"🆕 *Заказ аккаунта #{order['id']}*\n👤 @{order['username'] or order['user_id']} (id:{order['user_id']})\n📦 {order['product_name']}\n💰 {order['price']}₽\n📬 {order['address']}\n🕒 {order['created']}\n📸 Чек:"
    if order['screenshot']:
...
async def admin_stars_processing(c: types.CallbackQuery):
    ...
    for o in orders:
        text += f"┌ #{o['id']} | {o['price']}₽ | {o['stars']}⭐ | @{o['username'] or o['user_id']} (id:{o['user_id']})\n└ 📬 Начисление: {o['stars_username']}\n\n"
    kb = InlineKeyboardMarkup(inline_keyboard=[])
    for o in orders:
...
async def view_stars_order(c: types.CallbackQuery):
    ...
    if not order:
        await c.answer("Заказ не найден")
        return
    caption = f"⭐ *Заказ звёзд #{order['id']}*\n👤 @{order['username'] or order['user_id']} (id:{order['user_id']})\n⭐ {order['stars']} Stars\n💰 {order['price']}₽\n📬 Начисление: {order['stars_username']}\n🕒 {order['created']}\n📸 Чек:"
    if order['screenshot']:
...
async def admin_completed(c: types.CallbackQuery):
    ...
    for o in account_orders:
        created = o['created'].strftime("%d.%m %H:%M")
        text += f"┌ [#{o['id']}] Аккаунт | {o['price']}₽ | {o['product_name']} | id:{o['user_id']}\n└ 📅 {created} | 📬 {o['address']}\n\n"
    for o in stars_orders:
        created = o['created'].strftime("%d.%m %H:%M")
        text += f"┌ [#{o['id']}] Звёзды | {o['price']}₽ | {o['stars']}⭐ | id:{o['user_id']}\n└ 📅 {created} | 📬 {o['stars_username']}\n\n"
    kb = InlineKeyboardMarkup(inline_keyboard=[])
    nav_buttons = []
    if page > 0:
...
async def main():
    ...
    logger.info("Бот запущен (PostgreSQL + asyncpg, исправленный FSM)")
