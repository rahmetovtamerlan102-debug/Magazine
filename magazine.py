...
# Администраторы (личные ID или ID группы)
...
# Flask для пинга (чтобы Render не усыплял)
...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    # Короткое уведомление администраторам
...
async def reject_order_admin(callback: types.CallbackQuery):
    ...
    if callback.message.caption:
        new_caption = callback.message.caption.replace("📸 *Чек:*", "❌ **ОТКАЗАНО**")
        await callback.message.edit_caption(caption=new_caption, parse_mode="Markdown")
