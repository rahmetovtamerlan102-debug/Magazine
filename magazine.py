...
async def cmd_start(message: types.Message):
    ...
    if len(args) > 1 and args[1].startswith("order_"):
        order_text = unquote(args[1][6:])  # убираем "order_"
        # Отправляем уведомление админам
        for admin_id in ADMIN_IDS:
            await bot.send_message(
                admin_id,
                f"🛒 *НОВЫЙ ЗАКАЗ С САЙТА!*\n\n{order_text}\n\n⏳ Ожидает оплаты и скриншота",
                parse_mode="Markdown"
            )
        await message.answer(
            f"✅ Заказ с сайта принят!\n\n{order_text}\n\n"
            f"📌 Оплатите по реквизитам:\n"
            f"📱 СБП: `{PHONE_NUMBER}`\n"
            f"👤 Получатель: {RECIPIENT_NAME}\n\n"
            f"После оплаты отправьте СКРИНШОТ чека сюда — мы сразу выдадим товар.",
            parse_mode="Markdown"
        )
        return

    # ===== ОСТАЛЬНОЙ КОД (проверка подписки, меню и т.д.) =====
...
async def check_subscription(callback: types.CallbackQuery):
    if await is_subscribed(callback.from_user.id):
        await callback.message.edit_text(
            ...
            reply_markup=main_menu() if not is_admin(callback.from_user.id) else None
        )
...
# ... остальные обработчики (back_main, admin_panel, support, buy_number, select_country, buy_stars, select_stars, process_screenshot, invalid_screenshot, my_orders, admin_all_orders, admin_block, process_block_user, confirm_order_admin, reject_order_admin) остаются без изменений.
# Они уже есть в вашем коде и не требуют правки.
