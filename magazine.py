...
ADMIN_IDS = list(map(int, os.getenv("ADMIN_IDS", "8276815852").split(',')))
...
async def init_db():
    ...
    await db.execute('''CREATE TABLE IF NOT EXISTS subscription_check (
        user_id INTEGER PRIMARY KEY,
...
async def get_stats():
    ...
    cursor = await db.execute("SELECT status, COUNT(*) as count, SUM(product_price) as sum FROM orders GROUP BY status")
...
async def update_subscription_check(user_id):
    db = await get_db()
    await db.execute(
        "INSERT OR REPLACE INTO subscription_check (user_id, last_check) VALUES (?, strftime('%s', 'now'))",
        (user_id,)
    )
    await db.commit()
...
async def check_subscription(user_id):
    try:
        ...
        db = await get_db()
        cursor = await db.execute(
            "SELECT last_check FROM subscription_check WHERE user_id = ? AND last_check > strftime('%s', 'now') - 300",
            (user_id,)
        )
        ...
        if is_member:
            await update_subscription_check(user_id)
...
def subscription_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📢 ПОДПИСАТЬСЯ НА КАНАЛ", url=f"https://t.me/{REQUIRED_CHANNEL[1:]}")],
...
async def upload_check(request):
    try:
        data = await request.post()
        user_id = data.get('user_id')
        product_name = data.get('product_name')
        product_price = data.get('product_price')
        screenshot_file = data.get('screenshot')
        
        if not screenshot_file:
            return web.json_response({"status": "error", "error": "no file"}, status=400)
        if not user_id or not product_name or not product_price:
            return web.json_response({"status": "error", "error": "missing fields"}, status=400)
        
        photo_bytes = await screenshot_file.read()
        ...
        return web.json_response({"status": "ok", "message": "Чек отправлен админу"})
    except Exception as e:
        logger.error(f"Ошибка в upload_check: {e}")
        return web.json_response({"status": "error", "error": str(e)}, status=500)
...
async def run_aiohttp():
    ...
    app.router.add_post('/upload_check', upload_check)
        ...
        if user_id:
            if not await check_subscription(user_id):
                if isinstance(event, types.Message):
                    await event.answer(
                        "🔒 *ДОСТУП ЗАКРЫТ*\n\n"
                        f"Для использования магазина необходимо подписаться на наш канал:\n{REQUIRED_CHANNEL}\n\n"
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="◀ НАЗАД", callback_data="back_main")]  # Изменено на "◀ НАЗАД"
    ])
...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    if not await check_subscription(message.from_user.id):
        await message.answer(
            ...
            f"Для доступа к магазину необходимо подписаться на наш канал:\n{REQUIRED_CHANNEL}\n\n"
            "После подписки нажмите кнопку «ПРОВЕРИТЬ ПОДПИСКУ»",
            parse_mode="Markdown",
            reply_markup=subscription_keyboard()
        )
...
async def my_orders(callback: types.CallbackQuery):
    ...
    if not orders:
        ...
        return
    text = "📦 МОИ ЗАКАЗЫ\n\n"
    status_names = {"waiting_confirmation": "🔄 На проверке", "confirmed": "✅ Принят", "rejected": "❌ Отклонён"}
    for o in orders:
...
async def admin_logs(callback: types.CallbackQuery):
    ...
    for log in logs:
        ...
        if log['target_user']: text += f" | user:{log['target_user']}"
        if log['order_id']: text += f" | order:#{log['order_id']}"
        text += "\n"
...
async def process_search(message: types.Message, state: FSMContext):
    ...
    if query.isdigit():
        ...
        if order:
            status_name = {"waiting_confirmation": "🔄 На проверке", "confirmed": "✅ Принят", "rejected": "❌ Отклонён"}.get(order['status'], order['status'])
            created = datetime.fromtimestamp(order['created_at']).strftime("%d.%m.%Y %H:%M")
            src = "🌐" if order['source'] == 'website' else "🤖"
            text = f"📋 ЗАКАЗ #{order['id']}\n\n👤 @{order['username'] or 'нет username'} [ID: {order['user_id']}]\n📦 {order['product_name']}\n💰 {order['product_price']} ₽\n📌 Статус: {status_name}\n🕐 {created}\n{src} Источник: {'Сайт' if order['source']=='website' else 'Бот'}\n"
            if order['confirmed_by']: text += f"👑 Подтверждён админом: {order['confirmed_by']}\n"
...
async def admin_pending(callback: types.CallbackQuery):
    ...
    if not orders:
        ...
        return
    for o in orders:
        text = (f"📋 ЗАЯВКА #{o['id']}\n👤 @{o['username'] or 'без username'} [ID:{o['user_id']}]\n📦 {o['product_name']}\n💰 {o['product_price']} ₽\n🕐 {datetime.fromtimestamp(o['created_at']).strftime('%d.%m %H:%M')}\n🌐 Источник: {'Сайт' if o['source']=='website' else 'Бот'}\n\n📸 Чек:")
        if o['screenshot_file_id']:
            await callback.message.answer_photo(o['screenshot_file_id'], caption=text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ ПРИНЯТЬ", callback_data=f"confirm_{o['id']}"), InlineKeyboardButton(text="❌ ОТКАЗАТЬ", callback_data=f"reject_{o['id']}")]]))
        else:
            await callback.message.answer(text+"\n⚠️ Нет чека", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ ПРИНЯТЬ", callback_data=f"confirm_{o['id']}"), InlineKeyboardButton(text="❌ ОТКАЗАТЬ", callback_data=f"reject_{o['id']}")]]))
    try:
...
async def admin_export(callback: types.CallbackQuery):
    ...
    for o in orders:
        writer.writerow([o['id'], o['user_id'], o['username'], o['product_name'], o['product_price'], o['status'], o['source'], datetime.fromtimestamp(o['created_at']).strftime("%Y-%m-%d %H:%M:%S"), o['screenshot_file_id'] or "-"])
...
async def admin_block(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🚫 Введите ID пользователя и причину (через пробел):\nПример: `123456789 Нарушение правил`", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀ ОТМЕНА", callback_data="admin_panel")]]))
    await callback.answer()
...
async def admin_unblock(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🔓 Введите ID пользователя для разблокировки:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀ ОТМЕНА", callback_data="admin_panel")]]))
    await callback.answer()
...
async def confirm_order(callback: types.CallbackQuery):
    ...
    if not order:
        ...
        return
    await bot.send_message(order['user_id'], f"✅ ЗАКАЗ #{order_id} ПРИНЯТ!\n\n📦 {order['product_name']}\n💰 {order['product_price']} ₽\n🎉 Спасибо за покупку!\n\n📞 {SUPPORT_USERNAME}")
