...
ADMIN_IDS = [8276815852, 8840342301]   # сюда можно добавить и ID группы
...
async def test_notify(message: types.Message):
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(admin_id, "🔔 Тестовое уведомление от бота")
            await message.answer(f"✅ Отправлено админу {admin_id}")
        except Exception as e:
            await message.answer(f"❌ Ошибка админу {admin_id}: {e}")
...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    # === УВЕДОМЛЕНИЕ АДМИНАМ (с диагностикой) ===
    for admin_id in ADMIN_IDS:
        try:
