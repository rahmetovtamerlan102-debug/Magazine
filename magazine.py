...
# ЯВНЫЙ СПИСОК АДМИНИСТРАТОРОВ (ВАШ ID И ID КОЛЛЕГИ)
...
# Товары: виртуальные номера (страны)
COUNTRIES = {
    "us": {"name": "🇺🇸 США (+1)", "price": 80},
    "ca": {"name": "🇨🇦 Канада (+1)", "price": 65},
    "ru": {"name": "🇷🇺 Россия (новорег)", "price": 140, "warning": ""},
    ...
    "in": {"name": "🇮🇳 Индия", "price": 50, "warning": "⚠️ Высокий шанс слёта аккаунта!"}
}
...
async def buy_number(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "📱 *Выберите страну для номера Telegram:*",
        parse_mode="Markdown",
...
# -------------------- ПРИЁМ СКРИНШОТА И УВЕДОМЛЕНИЕ АДМИНАМ --------------------
...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    # Отправляем уведомление ВСЕМ администраторам с повторной попыткой
    caption = (
        ...
        f"➡️ Для подтверждения используйте кнопки:"
    )
...
async def confirm_order_admin(callback: types.CallbackQuery):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        order = conn.execute("SELECT user_id, product_price FROM orders WHERE id = ?", (order_id,)).fetchone()
    if order:
        uid, price = order
