...
# -------------------- Просмотр конкретного заказа (аккаунт) --------------------
...
# -------------------- Просмотр конкретного заказа (звёзды) --------------------
...
async def confirm_account_order(c: types.CallbackQuery):
    ...
    if order:
        ...
        try:
            await bot.send_message(
                user_id,
                f"✅ *Ваш заказ аккаунта #{order_id} подтверждён!*\n\n"
                f"📦 {order[4]}\n"
                f"💰 {order[5]}₽\n\n"
...
async def confirm_stars_order(c: types.CallbackQuery):
    ...
    if order:
        ...
        try:
            await bot.send_message(
                user_id,
                f"✅ *Ваш заказ звёзд #{order_id} подтверждён!*\n\n"
                f"⭐ {order[3]} Stars\n"
                f"💰 {order[4]}₽\n"
                f"📬 Начисление на: {order[5]}\n\n"
...
async def reject_account_order(c: types.CallbackQuery):
    ...
    if order:
        ...
        try:
            await bot.send_message(
                user_id,
                f"❌ *Ваш заказ аккаунта #{order_id} отклонён*\n\n"
...
async def reject_stars_order(c: types.CallbackQuery):
    ...
    if order:
        ...
        try:
            await bot.send_message(
                user_id,
                f"❌ *Ваш заказ звёзд #{order_id} отклонён*\n\n"
                f"Причина: чек не соответствует оплате или нечитаем.\n"
                f"Пожалуйста, отправьте корректный скриншот чека, начав заказ заново с /start",
                parse_mode="Markdown"
            )
...
# -------------------- Просмотр подтверждённых / отклонённых заказов --------------------
...
async def main():
    ...
    logger.info("Бот запущен с инлайн-клавиатурой и полной админ-панелью")
