...
async def verify_receipt(file_id: str, expected_amount: int) -> tuple[bool, str]:
    """Проверяет чек: номер телефона, ФИО, сумма, ключевые слова"""
    try:
        ...
        logger.info(f"OCR текст:\n{text}")
        
        # 1. Фейковые слова
        fake_words = ["ТЕСТ", "ПРИМЕР", "ДЕМО", "SAMPLE", "DEMO", "TEST", "ФЕЙК", "ПОДДЕЛКА"]
        ...
        # 3. Проверка номера телефона получателя
        phone_patterns = [
        ...
        logger.error(f"Ошибка проверки чека: {e}")
        return False, "❌ Техническая ошибка при проверке"
...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    if is_valid:
        # Чек настоящий — подтверждаем заказ
        ...
        for admin_id in ADMIN_IDS:
            await bot.send_photo(
                admin_id,
                photo.file_id,
                caption=f"✅ *АВТО-ПОДТВЕРЖДЕНИЕ* #{order_id}\n"
                        f"👤 @{message.from_user.username or 'без username'}\n"
                        f"📦 {data['product_name']}\n"
                        f"💰 {data['product_price']}₽\n"
                        f"📋 Чек проверен автоматически",
                parse_mode="Markdown"
            )
    else:
        # Чек фейковый — отклоняем
        ...
        # Уведомление админам о фейке
        for admin_id in ADMIN_IDS:
            await bot.send_photo(
                admin_id,
                photo.file_id,
                caption=f"⚠️ *ФЕЙКОВЫЙ ЧЕК* #{order_id}\n"
                        f"👤 @{message.from_user.username or 'без username'}\n"
                        f"📦 {data['product_name']}\n"
                        f"💰 {data['product_price']}₽\n"
                        f"❌ Причина: {reason}",
